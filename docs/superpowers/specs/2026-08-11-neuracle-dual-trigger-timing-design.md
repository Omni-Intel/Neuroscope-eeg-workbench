...
目标是让 NeuroScope 内所有现有范式使用同一打标接口、同一码表和同一审计格式，并能通过光电二极管或音频回环测量屏幕与扬声器端的真实刺激起始。
...
- 光电二极管与音频回环校准数据的保存与统计；
- JSONL、CSV 和 Excel 中文对照输出；
- 自动测试、博睿康台架测试和物理刺激校准流程。
...
负责预先打开串口并发送单字节事件码。每个非零码形成 5 ms 脉冲，再复位为 `0`。正式实验开始后不在事件路径中扫描端口、重新建连或等待用户输入。
...
SSVEP 不为每一帧发送硬件码，只标记稳定闪烁段的开始和结束。显示角落提供可配置的黑白光电同步块；校准时以光电通道为屏幕物理起始的权威依据。

### 6.2 听觉刺激

听觉输出改用持续的音频流，预先准备样本，避免每个 trial 临时创建播放链路。开始输出首个有效缓冲时提交硬件与 LSL 事件，并保存音频后端提供的输出时间信息。

声卡、驱动和扬声器仍会引入固定延迟与抖动。完成音频回环时，以 AUX 中实际波形起始为物理起始；未回环时只能标为硬件触发已记录、声音物理起始未校准。
...
`0` 专用于脉冲复位，`1–127` 用于事件。任何范式不得直接写裸数字，必须通过版本化码表的符号名获取。
...
| 121 | `PHOTODIODE_CALIBRATION` | 光电校准事件 |
| 122 | `AUDIO_LOOPBACK_CALIBRATION` | 音频回环校准事件 |
| 127 | `ABORT` | 中止或异常结束 |

未列出的码值在码表 v1 中保留，不得由范式临时占用。SSVEP 的具体频率保存在 LSL；硬件码对应本次会话中固定的目标索引，实际索引到频率的映射写入会话码表副本。当前 NeuroScope 运动想象只使用左手、右手和静息时，静息沿用公共 `REST=20`；四分类示例仍完整保留 `1–4`。
...
```json
{
  ...
  "timing_status": "hardware_sample_locked"
}
```
...
2. 校验单字节模式、事件码范围和 5 ms 复位设置；
3. 发送专用校准码 `120`；
4. 由实验人员确认博睿康 Trigger 通道收到校准码；
5. 建立 LSL Marker Outlet，并显示 Collect 接收状态；
6. 校验码表版本、EEG 采样率和记录目录可写；
7. 显示光电或音频回环的连接与校准状态。
...
- 事件重复、乱序或复位失败：记录具体序号和码值，不能在收尾时自动隐藏；
...
任何丢失、重复、未复位或错配都视为台架测试失败。绝对延迟和抖动不预先伪造阈值；首次台架数据形成基线后，实施阶段再根据采样率、显示刷新率和音频缓冲给出可解释的验收范围。

### 14.3 物理刺激校准

- 视觉：在不同屏幕位置重复呈现同步块，比较 Trigger 采样点与光电波形起始；
- 听觉：重复输出已知脉冲或短音，比较 Trigger 采样点与音频回环起始；
- 各自报告固定偏移、中位数、P95、P99、最大值和丢失数；
- 校准结果与显示器、刷新率、音频设备、采样率和软件版本绑定；
- 更换硬件或关键设置后重新校准。
...
- 台架测试可量化丢码、错码、延迟和抖动，物理校准可量化屏幕或扬声器端实际起始；
