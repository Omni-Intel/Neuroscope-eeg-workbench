...
def test_five_channel_decoders_use_fp_channels_and_keep_erp_gated() -> None:
    ...
    nback = PARADIGMS["N-back 工作记忆"].analyze(metadata, data, (event,))
...
def test_nback_decoder_reports_three_load_theta_differences() -> None:
    np = __import__("numpy")
    metadata = SourceMetadata.eeg("headband", "headband", 250.0, ("Fp1", "Fp2", "Fpz"))
    event = EEGEvent(
        1.0,
        "stimulus_event",
        "nback",
        {
            "nback_0_theta_db": 1.0,
            "nback_1_theta_db": 2.5,
            "nback_2_theta_db": 4.0,
            "nback_0_alpha_db": 8.0,
            "nback_1_alpha_db": 7.0,
            "nback_2_alpha_db": 6.0,
            "nback_0_beta_db": 3.0,
            "nback_1_beta_db": 3.5,
            "nback_2_beta_db": 4.0,
            "nback_0_behavior_accuracy": 0.95,
            "nback_1_behavior_accuracy": 0.90,
            "nback_2_behavior_accuracy": 0.80,
        },
    )

    result = PARADIGMS["N-back 工作记忆"].analyze(
        metadata, np.zeros((3, 1000), np.float32), (event,)
    )

    assert result.metrics["1-back - 0-back theta dB"] == 1.5
    assert result.metrics["2-back - 0-back theta dB"] == 3.0
    assert result.metrics["2-back 正确率"] == 0.8
