...
- 2-back 工作记忆：10 个练习后运行 30/60 个正式试次，展示行为、前额 theta 和 Fp1/Fp2 偏侧趋势
- Stroop 色词冲突：12 个练习后运行 30/60 个正式试次，展示正确率、反应时干扰量和前额 theta/beta
- 情绪图片唤醒：自有素材七类各 15 张，记录效价/唤醒评分并展示 Fp1/Fp2 alpha 偏侧趋势
- 听觉 ASSR：40 Hz 调幅音刺激和 T3/T4 优先的频率跟随 SNR
- 听觉 Oddball：10 个练习后运行 100/200 个 80/20 标准音与偏差音，展示行为和软件事件记录
...
需要运行内置范式时，先启动采集，再选择任务范式、协议预设和第二块显示器，点击“开始刺激”。`快速演示` 减少阶段时长或正式试次，`完整采集` 使用 60 个 2-back、60 个 Stroop、200 个 Oddball 和全部 105 张情绪图片。练习事件会标记为 `is_practice=true`，不进入正式行为统计。按 `Esc` 可随时退出刺激，事件可导出为 CSV。

情绪图片流程包含内容提示、20 秒中性基线、1 秒注视、6 秒图片、1–9 效价评分、1–9 唤醒评分和结束缓和页。七个细分类为愉悦、厌恶、恐惧、鼓舞、中性、悲伤、温情，每类 15 张；这些素材是公司自有素材，不标注为 IAPS。按 `S` 可以跳过当前图片。

当前没有硬件 Trigger 时使用同机软件时间戳同步。界面中的“候选分离度”和“趋势分”不是模型准确率；SSVEP 只有完成带提示目标的试次后才显示本会话试次匹配率。

听觉 ASSR 使用 10 秒安静基线和 20 秒 40 Hz 调幅音，可用于观察频谱中的 40 Hz 跟随趋势。听觉 Oddball 使用 80% 标准音和 20% 偏差音，听到偏差音时按空格。两项听觉任务建议准备同一副舒适音量的双耳耳机。没有硬件事件标记或尚未校准设备时间戳与电脑音频/显示延迟时，Oddball 的 N1/MMN/晚期正波以及 Stroop 的 N2/N450 均只显示“ERP 时序待校准”，不输出不可靠数值。
